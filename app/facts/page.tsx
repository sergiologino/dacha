...
import { getPublishedFunFactsWithFallback } from "@/lib/queries/guide-content";
...
export default async function FactsPage() {
  const facts = await getPublishedFunFactsWithFallback();
  const categories = await prisma.factCategory
  ...
  if (facts.length === 0) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center text-slate-600 dark:text-slate-400">
        <p className="mb-2">Не удалось загрузить факты. Попробуйте обновить страницу позже.</p>
      </div>
    );
  }
