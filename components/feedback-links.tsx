...
import { MaxMessengerIcon } from "@/components/icons";
import { cn } from "@/lib/utils";

type FeedbackLinksProps = { variant?: "bar" | "inline" | "icons" };

export function useFeedbackUrls(): { max: string } {
  const [links, setLinks] = useState<{ max: string }>({ max: "" });

  useEffect(() => {
    fetch("/api/config")
      .then((r) => r.json())
      .then((data: { feedbackMaxUrl?: string }) => {
        setLinks({
