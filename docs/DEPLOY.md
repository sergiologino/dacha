...
**Если провайдер не умеет тома** — фото после перезапуска снова будут теряться; имеет смысл либо сменить тип сервиса (VPS + свой Docker), либо позже вынести файлы в объектное хранилище (отдельная доработка).
