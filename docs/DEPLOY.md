...
- **Дата события** — «сегодня» и «завтра» считаются по часовому поясу `PUSH_REMINDERS_TZ` (по умолчанию Europe/Moscow). Событие должно быть с `isAction: true` и без `doneAt`.
