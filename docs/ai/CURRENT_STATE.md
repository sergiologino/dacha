...
**Миграция БД после деплоя**: если в таблице `crops` ещё нет колонки `varieties`, выполнить `npx prisma db push` или `npx prisma migrate deploy` с `DATABASE_URL` продакшена. До этого справочник покажет только статические культуры; после миграции заработает «Добавить в справочник» и список культур из БД.
...
## Что НЕ реализовано
- [ ] **Учёт платежей и тарифов** — хранить и отображать успешные/неуспешные платежи: кто, когда, сумма, план, статус (succeeded/canceled/failed), причина ошибки; веб-интерфейс или отчёт (админка/страница «Платежи»). Модель `Payment` в БД есть, запись при создании платежа и обновление по webhook ЮKassa — не реализованы.
