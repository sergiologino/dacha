...
- [x] **Календарь** `/calendar` — 50+ задач на 12 месяцев, 5 категорий, переключатель месяцев, ссылки на справочник; **запланированные работы с грядок** — события таймлайна ухода (PlantTimelineEvent) по выбранному месяцу отображаются блоком «Запланированные работы» над общими задачами, с датой, грядкой, культурой и составом работ; карточки выделены цветом (emerald) и тэгом «С грядок» для отличия от общих задач
